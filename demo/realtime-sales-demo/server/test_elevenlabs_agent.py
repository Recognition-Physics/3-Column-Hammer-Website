"""ElevenLabs custom LLM — Hammer sales instructions only."""
...
class ElevenLabsScenarioTests(unittest.TestCase):
    def test_voice_scenario_always_hammer(self) -> None:
        self.assertEqual(_voice_scenario({}), "hammer")
        self.assertEqual(
            _voice_scenario({"custom_llm_extra_body": {"voice_scenario": "hammer"}}),
            "hammer",
        )
        self.assertEqual(
            _voice_scenario({"elevenlabs_extra_body": {"voice_scenario": "pen"}}),
            "hammer",
        )
        body = {
    ...
    def test_opening_greeting_hammer_product_focus(self) -> None:
        greeting = _opening_greeting("hammer", "Hammer Drive")
        self.assertIn("Hammer Drive", greeting)
        self.assertIn("Hannah", greeting)

    def test_prompt_always_uses_hammer_sales_instructions(self) -> None:
        session = CallSession()
        ...
        pen_labeled = _build_prompt(session, "", "pen")
        self.assertIn("You are Hannah", pen_labeled)
        self.assertNotIn("Sell Me This Pen", pen_labeled)

    def test_hammer_prompt_includes_pricing_and_readback(self) -> None:
        session = CallSession()
        prompt = _build_prompt(session, "── PRODUCT CONTEXT ──\nSample wiki.", "hammer")
