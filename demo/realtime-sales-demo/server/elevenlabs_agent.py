...
All tool schemas match the SIP/phone path; browser and phone both use hammer_browser_tool_definitions.
"""
...
def _voice_scenario(body: dict) -> str:
    """All voice sessions use Hammer sales instructions."""
    return "hammer"
...
def _hammer_product_from_extra(body: dict) -> str:
    extra = _llm_extra_body(body)
    return str(extra.get("hammer_product") or "").strip()


def _apply_hammer_product_focus(session: CallSession, body: dict, voice_scenario: str) -> None:
    if voice_scenario != "hammer":
        return
    product = _hammer_product_from_extra(body)
    if not product:
        return
    session.pen_buyer_product = product
    if not session.agreement_plan:
        session.agreement_plan = product


def _opening_greeting(voice_scenario: str, hammer_product: str = "") -> str:
    """Instant spoken greeting for the first custom-LLM turn (before any user speech)."""
    if hammer_product:
        return f"Hey it's Hannah with Hammer — want to hear about {hammer_product}?"
...
def _session_state_block(session: CallSession) -> str:
    ...
    context window."""
    lines: list[str] = []
    if session.voice_scenario == "hammer" and session.pen_buyer_product:
        product = session.pen_buyer_product
        lines.append(
            f'PRODUCT FOCUS: Visitor clicked the "{product}" card on the website. '
            f"Open with a short pitch on {product} — what it does, pricing, and who it fits. "
            f'Default selected_plan for capture_lead: "{product}". '
            "Only discuss other Hammer products if they ask."
        )
...
def _build_prompt(session: CallSession, wiki_context: str, voice_scenario: str) -> str:
    """Build the full Hannah system prompt — always Hammer sales instructions."""
...
async def handle_elevenlabs_llm(body: dict, get_retriever_fn: Callable) -> StreamingResponse:
    ...
    if _is_opening_turn(messages):
        hammer_product = _hammer_product_from_extra(body)
        greeting = _opening_greeting(voice_scenario, hammer_product)
        ...
        if call_id:
            ...
            _apply_hammer_product_focus(opener, body, voice_scenario)
    ...
    _apply_hammer_product_focus(session, body, voice_scenario)
    session.lead.channel = session.lead.channel or _elevenlabs_channel(body, voice_scenario)
    call_id = _call_id_from_body(body) or session.call_id
    if call_id:
    ...
    async def _generate() -> AsyncIterator[bytes]:
        ...
        try:
            ...
            executor = await _get_executor(get_retriever_fn, browser=True)
            ...
            tools = hammer_browser_chat_tool_definitions()
