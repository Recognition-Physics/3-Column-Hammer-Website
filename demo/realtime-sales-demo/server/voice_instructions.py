...
def build_phone_accept_instructions() -> str:
    """Slim prompt for SIP accept (faster pickup). Full Hammer sales rules pushed on sideband."""
    ...
    return (
        ...
        "── PHONE CALL — OPENING ONLY (full Hammer sales rules load during connect) ──\n"
        "When this call connects, you speak first — one unbroken breath with no period after Hey. "
        'Say "Hey it\'s Hannah with Hammer — what\'s on your mind?" or '
        '"Hannah here with Hammer — go ahead and ask me anything."\n'
        "Deliver the greeting once, then stop and wait for their reply.\n"
        "Never repeat this opening later in the call.\n"
        f"{disclosure_note}"
    )


def format_hammer_phone_opening_response_create() -> str:
    """Instructions for the first response.create on SIP — Hannah Hammer sales opener."""
    return (
        "Speak exactly once, one flowing phrase with no period after Hey:\n"
        '"Hey it\'s Hannah with Hammer — what\'s on your mind?"\n'
        "Or: \"Hannah here with Hammer — go ahead and ask me anything.\"\n"
        "Then stop completely and wait for their reply. Do not pitch products on this turn."
    )


def build_hammer_call_instructions(wiki_context: str = "") -> str:
    """SIP/OpenAI Realtime — Hammer sales persona only (same core as browser demo)."""
    return build_hammer_browser_prompt(wiki_context or "")


def format_phone_opening_response_create(opening: PenOpening) -> str:
    """Deprecated pen opener — kept for tests; production SIP uses format_hammer_phone_opening_response_create."""
    return format_hammer_phone_opening_response_create()
...
def format_austin_session_clock() -> str:
    ...
    return (
        ...
        "You may sign them up on this call if they prefer.\n"
