...
from voice_instructions import (
    build_hammer_call_instructions,
    build_phone_accept_instructions,
    format_hammer_phone_opening_response_create,
...
from voice_tools import CallSession, VoiceToolExecutor, hammer_browser_tool_definitions, parse_tool_arguments
...
class SipRealtimeService:
    ...
    def _build_accept_payload(self) -> dict[str, Any]:
        ...
        payload: dict[str, Any] = {
            **self._build_slim_accept_payload(),
            "tools": hammer_browser_tool_definitions(),
            "tool_choice": "auto",
    ...
    async def _run_sideband(self, call_id: str, *, caller_phone: str = "", call_direction: str = "") -> None:
        ...
        try:
            ...
            register_active_session(call_id, {"channel": "phone", "scenario": "hammer"})
        ...
        try:
            async with ws:
                ...
                await self._push_full_session_instructions(ws, session.wiki_context)
                await self._send_opening_greeting(ws)
                async for raw in ws:
                    try:
    ...
    async def _push_full_session_instructions(self, ws: Any, wiki_context: str) -> None:
        """Full session after slim accept — Hammer sales rules and signup tools."""
        fields: dict[str, Any] = {
            "instructions": build_hammer_call_instructions(wiki_context),
            "tools": hammer_browser_tool_definitions(),
            "tool_choice": "auto",
            "parallel_tool_calls": True,
            "output_modalities": ["audio"],
    ...
    async def _send_opening_greeting(self, ws: Any) -> None:
        await ws.send(
            json.dumps(
                {
                    "type": "response.create",
                    "response": {
                        "instructions": format_hammer_phone_opening_response_create(),
                    },
                }
            )
        )
            ...
            try:
                ...
                if session.call_id:
                    register_active_session(
                        session.call_id,
                        {"channel": "phone", "scenario": "hammer", "values": dict(session.lead.values or {})},
                    )
                    upsert_call_record(session.lead)
    ...
    async def _finish_opening_phase(self, ws: Any, session: CallSession) -> None:
        ...
        logger.info(
            ...
            "waiting for caller reply"
        )
        await self._enable_conversation_turn_detection(ws)
        # Do not response.create here — caller responds to Hannah's greeting first.
