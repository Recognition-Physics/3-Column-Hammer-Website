...
export function startVoiceAudioVisualizer(
  ...
  function tick(): void {
    ...
    const micReactiveCtas = root.querySelectorAll<HTMLElement>(".landing-cta.mic-reactive, .footer-cta__pill.mic-reactive");
    ...
    const productStage = root.querySelector<HTMLElement>(".product-col-voice-stage--live");
    const productViz = root.querySelector<HTMLElement>(
      ".product-col-voice-stage--live .voice-audio-visualizer",
    );
    const productOrb = root.querySelector<HTMLElement>(".product-col-voice-orb");
    const productGlow = root.querySelector<HTMLElement>(".product-col-voice-orb__glow");
    ...
    if (productStage) {
      productStage.style.setProperty("--voice-amp", amp);
      productStage.classList.toggle("is-assistant-speaking", assistant);
      productStage.classList.toggle("is-user-listening", activity === "listening");
    }
    if (productViz) {
      productViz.style.setProperty("--volumeter-scale", scaleStr);
      productViz.style.setProperty("--volumeter-brightness", brightStr);
      productViz.style.setProperty("--voice-amp", amp);
      productViz.classList.toggle("is-activity-listening", activity === "listening");
      productViz.classList.toggle("is-activity-speaking", activity === "speaking");
    }
    if (productGlow) {
      productGlow.style.setProperty("--voice-amp", amp);
      productGlow.classList.toggle("is-user-listening", activity === "listening");
      productGlow.classList.toggle("is-assistant-speaking", assistant);
    }
  ...
  return {
    stop() {
      ...
      for (const el of [
        ...
        root.querySelector<HTMLElement>(".product-col-voice-stage--live"),
        root.querySelector<HTMLElement>(".product-col-voice-stage--live .voice-audio-visualizer"),
        root.querySelector<HTMLElement>(".product-col-voice-orb"),
        root.querySelector<HTMLElement>(".product-col-voice-orb__glow"),
      ]) {
        if (!el) continue;
